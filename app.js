...
const Synth = {
  ...
  // 門磨擦阻力與喀啦聲 (由多個微小的雜音與音調滑音組成)
  playDoorSlide(open = true) {
    this.init();
    if (this.ctx.state === "suspended") {
      this.ctx.resume();
    }
    ...
    for (let i = 0; i < 8; i++) {
      ...
      osc.type = "triangle";
      // 開門音調上升，關門下降
      const startFreq = open ? 80 + i * 15 : 200 - i * 15;
      osc.frequency.setValueAtTime(startFreq, now + timeOffset);
      osc.frequency.exponentialRampToValueAtTime(startFreq + (open ? 10 : -10), now + timeOffset + 0.06);
      
      gain.gain.setValueAtTime(0.06, now + timeOffset);
      gain.gain.linearRampToValueAtTime(0.0001, now + timeOffset + 0.06);
    ...
    if (!open) {
      setTimeout(() => {
        this.playBeep(120, 0.05, "square", 0.15); // low clack
        setTimeout(() => {
          this.playBeep(90, 0.08, "triangle", 0.1); // deep clack
        }, 50);
      }, 600);
    }
  }
...
// 新增：木紋拉門 DOM 元素
const tvDoorLeft = document.getElementById("tv-door-left");
const tvDoorRight = document.getElementById("tv-door-right");
...
async function loadAthleteHistory(userId) {
  ...
  try {
    ...
    // 更新統計面板
    if (totalRepsStat) totalRepsStat.textContent = stats.total_reps || 0;
    if (avgScoreStat) avgScoreStat.textContent = stats.avg_score || 0;
    if (totalWorkoutsStat) totalWorkoutsStat.textContent = stats.total_workouts || 0;
...
function renderEmptyHistory() {
  ...
  if (totalRepsStat) totalRepsStat.textContent = "0";
  if (avgScoreStat) avgScoreStat.textContent = "0";
  if (totalWorkoutsStat) totalWorkoutsStat.textContent = "0";
}

// 儲存當前動作紀錄到資料庫
async function submitWorkoutSession(isLastOfRoutine = false) {
  if (!currentUserId) {
    console.warn("未選取運動員，將做操數據紀錄於主控台。");
    return;
  }
...
async function toggleCamera() {
  if (isCameraActive) {
    ...
    if (tvDoorLeft && tvDoorRight) {
      tvDoorLeft.classList.remove("opened");
      tvDoorRight.classList.remove("opened");
      if (chkSfx.checked) Synth.playDoorSlide(false);
      // 等門關上後再釋放相機與 AI 資源
      setTimeout(() => {
        stopCamera();
      }, 800);
    ...
    Synth.init();
    if (tvDoorLeft && tvDoorRight) {
      tvDoorLeft.classList.add("opened");
      tvDoorRight.classList.add("opened");
      if (chkSfx.checked) Synth.playDoorSlide(true);
      // 等門拉開一半以上再開啟相機
      setTimeout(async () => {
        await startCamera();
      }, 600);
...
function detectionLoop() {
  ...
  if (webcamElement.readyState >= 2 && webcamElement.videoWidth > 0) {
    try {
      ...
      console.warn("MediaPipe detectForVideo error (will retry):", err);
    }
  }
  ...
  if (result && result.poseLandmarks && result.poseLandmarks.length > 0) {
    ...
    if (lm.length >= 29) {
      // 驗證必要點位存在且包含 x, y 座標，避免後續計算發生 TypeError 導致偵測中斷
      const requiredIds = [LS_ID, RS_ID, LE_ID, RE_ID, LW_ID, RW_ID, LH_ID, RH_ID, LK_ID, RK_ID, LA_ID, RA_ID];
      const allValid = requiredIds.every(id => lm[id] && typeof lm[id].x === "number" && typeof lm[id].y === "number");
      
      if (!allValid) {
        repStatus.textContent = "偵測不全";
        repStatus.style.color = "var(--vintage-yellow)";
        feedbackList.innerHTML = `<div class="feedback-placeholder">請確保全身 (肩膀至腳踝) 皆在畫面中，以便 AI 進行動作判定...</div>`;
        
        cachedState.landmarks = lm; // 仍保留 landmarks 供繪製骨骼
        cachedState.score = 30;
        cachedState.perfect = false;
        cachedState.feedback = ["請確保全身均在相機範圍內，以利精準偵測"];
...
function bindEvents() {
  ...
  // 點擊拉門直接開關電視
  if (tvDoorLeft && tvDoorRight) {
    tvDoorLeft.addEventListener("click", toggleCamera);
    tvDoorRight.addEventListener("click", toggleCamera);
  }
}
